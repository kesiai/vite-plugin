...
const initialUsers: User[] = [
  { id: 1, name: '张伟', role: '运维工程师', city: '北京', email: 'zhangwei@kesi.local', online: true },
  { id: 2, name: '李娜', role: '平台管理员', city: '上海', email: 'lina@kesi.local', online: true },
  { id: 3, name: '王强', role: '数据分析师', city: '深圳', email: 'wangqiang@kesi.local', online: false },
  { id: 4, name: '赵敏', role: '设备巡检员', city: '成都', email: 'zhaomin@kesi.local', online: true },
  { id: 5, name: '陈晨', role: '安全审计员', city: '武汉', email: 'chenchen@kesi.local', online: false },
];
...
export default function Users() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) =>
      [u.name, u.role, u.city, u.email].some((v) => v.toLowerCase().includes(kw))
    );
  }, [users, keyword]);
  ...
  const confirmDelete = () => {
    ...
    toast.add({
      type: 'success',
      title: '已删除用户',
      description: `${pendingDelete.name} 已从列表中移除（演示）。`,
    });
            ...
            <Card key={u.id}>
              <CardContent className="flex flex-wrap items-center justify-between gap-3 py-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant={u.online ? 'default' : 'secondary'}
                        className={
                          u.online
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="sm" onClick={() => editDemo(u)}>
                    <PencilIcon />
                    编辑
                  </Button>
                  <Button
